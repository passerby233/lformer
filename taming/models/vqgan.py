...
from third_party.diffusionmodules.model import Encoder, Decoder
...
class VQModel(pl.LightningModule):
    ...
    def decode_code(self, code_t):
        ...
        quant_b = self.quantize.get_codebook_entry(code_t)
    ...
    def log_images(self, batch, **kwargs):
        ...
        x = x.to(self.device)
        xrec, _ = self(x)
        log["inputs"] = self.de_normalize(x)
        log["reconstructions"] = self.de_normalize(xrec)
        return log
...
class GumbelVQ(VQModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        discloss, log_dict_disc = self.loss(qloss, x, xrec, 1, self.global_step,
                                            last_layer=self.get_last_layer(), split="val")
        rec_loss = log_dict_ae["val/rec_loss"]
        log_loss_dict = {"val/rec_loss": rec_loss, "val/aeloss": aeloss}
...
class ShrinkVQ(GumbelVQ):
    def __init__(self,
        ...
        if self.quantize_number != 0 and mapping_path!=None:
            full_to_quantize = torch.load(mapping_path)
            quantize_to_full = torch.zeros(self.quantize_number)-1
            for idx, i in enumerate(full_to_quantize):
                if quantize_to_full[i] == -1:
                    quantize_to_full[i] = idx
            quantize_to_full = quantize_to_full.long()
            self.register_buffer("full_to_quantize", full_to_quantize)
            self.register_buffer("quantize_to_full", quantize_to_full)

    def encode(self, x):
        h = self.encoder(x)
        h = self.quant_conv(h)
        quant, emb_loss, indices = self.quantize(h)
        indices = self.full_to_quantize[indices]
        return quant, emb_loss, indices

    def decode_code(self, code_t):
        code_t = self.quantize_to_full[code_t]
        if len(code_t.shape) == 2:
            s = int(math.sqrt(code_t.shape[1]))
            code_t = code_t.view(code_t.shape[0], s, s)
        quant_b = self.quantize.get_codebook_entry(code_t)
