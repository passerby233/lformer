...
class Extractor:
    ...
    def extract(self, config, mode='val', save_prefix=None):
        ...
        for batch in pbar:
            for img_key, data_key in zip(img_key_list, data_key_list):
                ...
                for batch_idx in range(len(indexes)):
                    ...
                    img_id = dataset.labels["image_ids"][global_idx]
                    data_dict[data_key][img_id] = img_tokens[batch_idx]
            
        data_dict['img_ids'] = dataset.labels["image_ids"]
        data_dict['img_id_to_captions'] = dataset.img_id_to_captions
        data_dict['img_id_to_filepath'] = dataset.img_id_to_filepath
...
def main():
    ...
    extractor.extract(config.data, args.mode, args.save_prefix)
